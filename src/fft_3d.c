/* ---------------------------------------------------------------------
   ...
   Copyright 2011 National Technology & Engineering Solutions of
...
// Details for doing a pack/unpack
...
// Fortran wrappers to perform a 3d FFT
...
void FTN(fft_3d_ip)(FFT_DATA *inout, int *flag, struct fft_plan_3d **plan)
{
  fft_3d_cfunc(inout,inout,*flag,*plan);
}
...
/* post-remap to put data in output format if needed
  ...
  if (plan->post_plan)
    remap_3d_cfunc((double *) data, (double *) out, (double *) plan->scratch,
	     plan->post_plan);
...
/*
   Arguments to create a plan for performing a 3d FFT
...
// Fortran wrapper to create a plan for performing a 3d FFT
...
// Create a plan for performing a 3d FFT
...
/* remap from initial distribution to layout needed for 1st set of 1d FFTs
  ...
  if (remapflag == 0) {
    ...
    plan->pre_plan =
      remap_3d_create_plan_cfunc(comm,in_ilo,in_ihi,in_jlo,in_jhi,in_klo,in_khi,
			   first_ilo,first_ihi,first_jlo,first_jhi,
			   first_klo,first_khi,howmany,
			   FFT_PRECISION,0,0,2);
...
/* remap from 1st to 2nd FFT
  ...
  plan->mid1_plan =
      remap_3d_create_plan_cfunc(comm,
			   first_ilo,first_ihi,first_jlo,first_jhi,
			   first_klo,first_khi,
			   second_ilo,second_ihi,second_jlo,second_jhi,
			   second_klo,second_khi,howmany,
			   FFT_PRECISION,1,0,2);
...
*/
  ...
  plan->fftsize = (out_ihi-out_ilo+1) * (out_jhi-out_jlo+1) *
      (out_khi-out_klo+1);
...
/* system specific pre-computation of 1d FFT coeffs and scaling normalization */
...
// Fortran wrapper to destroy a 3d fft plan
...
{
  remap_3d_cfunc(in,out,buf,*plan);
}
...
{
  remap_3d_destroy_plan_cfunc(*plan);
}
